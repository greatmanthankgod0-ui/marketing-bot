...
def industry_keyboard():
    kb = ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True)
    kb.row(KeyboardButton("🛢️ Oil & Gas"), KeyboardButton("🧵 Fabric & Fashion"))
...
def seller_dashboard(message):
    ...
    label = "🛢️ Oil & Gas" if oil else "🧵 Fabric"
...
def cmd_myprofile(message):
    ...
    conn = get_conn()
    c = conn.cursor()
    c.execute("SELECT full_name, phone, address, tc_agreed, joined_at, total_orders, total_spent FROM buyer_profiles WHERE user_id = ?", (user_id,))
    b = c.fetchone()
    conn.close()
    if not b:
...
def start_oil_seller_reg(message):
    ...
    conn.close()
    bot.send_message(user_id,
        f"🛢️ *Setting up your OnTabs Oil & Gas Shop!*\n\n"
        f"First, read and accept the Seller Agreement:\n"
        f"{SELLER_TC}\n\n"
        f"Type *AGREE* to accept and continue.",
        parse_mode="Markdown",
...
def handle_seller_reg(message, oil=False):
    ...
    if state == "ASK_BIO":
        ...
        if oil:
            ...
            c3.execute("""INSERT OR REPLACE INTO oil_sellers
                      ...
                      (user_id, s[0], company_name, s[1], s[2], s[3], s[4],
                       datetime.now().isoformat(), datetime.now().isoformat(), bio))
        else:
        ...
        industry_label = "Oil & Gas" if oil else "Fabric"
...
def handle_receipt(message):
    ...
    c.execute(f"UPDATE {tbl_o} SET status = 'PAID', paid_at = ?, release_at = ?, receipt_msg_id = ? WHERE reference = ?",
              (datetime.now().isoformat(), release_at.isoformat(), message.message_id, ref))
...
def main_handler(message):
    ...
    # Approved seller
    if (is_seller(user_id) or is_seller(user_id, oil=True)) and lower not in ["🛍️ i want to buy", "i want to buy"]:
        seller_dashboard(message)
        return
...
def handle_buyer(message):
    ...
    # ── Industry selection ──
    if lower == "🛢️ oil & gas" and (not session or session[0] in ("BROWSING", "START", None, "TC_PENDING")):
...
def show_sellers_by_industry(message, user_id, industry):
    ...
    if oil and shops:
        ...
        conn3.close()
        if products:
            lines2 = [f"🛢️ *{shops[0][1]} — Available Products:*\n"]
            for p in products:
