...
def handle_buyer(message):
    ...
    # ── BUY — ask for delivery address first ──
    if "buy" in lower and session and session[0] == "CONFIRMING":
        conn = get_conn()
        c = conn.cursor()
        c.execute("UPDATE buyer_sessions SET state = 'WAITING_FOR_ADDRESS' WHERE user_id = ?", (user_id,))
        conn.commit()
        conn.close()
        bot.reply_to(message,
            "📍 *Almost there!*\n\n"
    ...
    if session and session[0] == "WAITING_FOR_ADDRESS":
        conn = get_conn()
        c = conn.cursor()
        c.execute("UPDATE buyer_sessions SET state = 'WAITING_FOR_PHONE', current_category = ? WHERE user_id = ?",
                  (text, user_id))
        conn.commit()
        conn.close()
        bot.reply_to(message,
            "📞 *Last step!*\n\n"
            "What's your *phone number* so the seller and dispatch rider can reach you?\n\n"
            "Example: 08012345678",
            parse_mode="Markdown"
        )
        return

    # ── WAITING FOR PHONE — then create order and send payment details ──
    if session and session[0] == "WAITING_FOR_PHONE":
        buyer_phone   = text
        delivery_addr = session[1]  # saved in current_category temporarily
        p_id          = session[3]
        qty           = session[4]
        seller_id     = session[5]
        ...
        bot.send_message(ADMIN_ID,
            ...
            f"Phone: {buyer_phone}\n"
            f"Item: {color} {category} x{qty}\n"
            f"Total: ₦{total:,.0f} | Fee: ₦{fee:,.0f} | Seller gets: ₦{seller_gets:,.0f}\n\n"
            f"📍 {delivery_addr}",
            parse_mode="Markdown"
        )
        return
